...
for(it in 6:6){
	...
	for(car_case_perc in clist){
		...
		# Polygenic background in full data
            #################################################################
        # randomize polygenic background 
        
        sub1 <- dfull[dfull$CASE == "TBD",]
        sub1$CASE <- "CTRL"
        sub1$CASE[sample(nrow(sub1), noncarr_cases)] <- "CASE"
        sub2 <- dfull[dfull$CASE != "TBD",]
        polydf <- rbind(sub1, sub2)
        names(polydf)[4] <- "FINALCASE"
		...
		png(paste0("/home/552/sn0095/shared/SarcomaAnalysis/Results/Simulation/SimulationCASES_prev",prev,"_Penet",car_case_perc,"it_",it,"_RANDOM.png"), res=350, units = "in", height = 4, width = 8)
		f <- plot_grid(box_cases, pf)
		print({f})
		dev.off()

		png(paste0("/home/552/sn0095/shared/SarcomaAnalysis/Results/Simulation/SimulationALL_prev",prev,"_Penet",car_case_perc,"it",it,"_RANDOM.png"), res=350, units = "in", height = 4, width = 12)
	...
	resdf$iteration <- it
    write.table(resdf, paste0("/home/552/sn0095/shared/SarcomaAnalysis/Results/Simulation/Simulation_prev",prev,"_Penet",car_case_perc,"_it",it,"fullresults_RANDOM.txt"), 
            row.names=F, col.names=T, quote=F, sep = "\t")
	fullres <- rbind(fullres, resdf)
}
# final plot 
r1 <- fullres
write.table(fullres, paste0("/home/552/sn0095/shared/SarcomaAnalysis/Results/Simulation/Simulation_prev",prev,"_Penet",car_case_perc,"fullresults_RANDOM_round3.txt"), 
            row.names=F, col.names=T, quote=F, sep = "\t")
...
write.table(f1, paste0("/home/552/sn0095/shared/SarcomaAnalysis/Results/Simulation/Simulation_prev",prev,"_Penet",car_case_perc,"Summaryresults_RANDOM_round3.txt"), 
            row.names=F, col.names=T, quote=F,sep = "\t")
...
px <- ggplot(f1, aes(x=Penetrance, y=MeanPRSdiff_c_nc)) + geom_point(aes(size=OR_carrier)) + #geom_line() +
  ...
  ggtitle("", subtitle = paste0("n=",tot,", Prev=",prev,", \nCarriers=",carrier_perc,", PRSrisk=4x")) + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, size=11, hjust=1),
        axis.text.y = element_text(size=11, hjust=1)) 

png(paste0("/home/552/sn0095/shared/SarcomaAnalysis/Results/Simulation/Simulation_prev",prev,"_Penet",car_case_perc,"Summaryresults_RANDOM_round3.png"), res=350, units = "in", height = 4, width = 6)
print({px})
dev.off() 
